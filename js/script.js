...
window.addEventListener("scroll", function () {
	// If popup was already closed before, never show it
	if (localStorage.getItem("calendarPopupClosed")) return;
	...
	if (!scrollTimerStarted) {
		...
		setTimeout(function () {
			if (!localStorage.getItem("calendarPopupClosed")) {
				document.getElementById("calendar-popup").style.display = "flex";
			}
		}, 10000); // 10 seconds after first scroll
	}
});
...
function closePopup() {
	...
	localStorage.setItem("calendarPopupClosed", "true"); // Save permanently
}


// Zoom toggle for image
const popupImg = document.querySelector(".calendar-popup img");
